...
required_vars=(DOMAIN PUBLIC_ORIGIN ACME_EMAIL LICENSE_TOKEN LICENSE_SERVER_URL \
...
# PUBLIC_ORIGIN matches
[ "$PUBLIC_ORIGIN" = "https://$DOMAIN" ] && pass "PUBLIC_ORIGIN matches DOMAIN" \
  || warn "PUBLIC_ORIGIN ($PUBLIC_ORIGIN) does not match https://$DOMAIN"
...
# Caddy cert acquired?
if $COMPOSE ps edge 2>/dev/null | grep -q "running"; then
...
if curl -fsS --max-time 10 "https://$DOMAIN/api/health" >/dev/null 2>&1; then
  pass "HTTPS reachable: https://$DOMAIN/api/health"
else
  fail "https://$DOMAIN/api/health did not respond" "check: TLS cert, edge container, firewall on 443"
fi

# Dashboard reachable
if curl -fsS --max-time 10 -o /dev/null "https://$DOMAIN/dashboard/"; then
  pass "dashboard reachable: https://$DOMAIN/dashboard/"
else
  fail "dashboard not reachable" "edge may be misrouting — see: hzaconnect logs edge"
fi

# widget.js reachable + CORS
cors=$(curl -fsS --max-time 10 -I "https://$DOMAIN/widget.js" 2>/dev/null | tr -d '\r' | awk -F': ' 'tolower($1)=="access-control-allow-origin"{print $2}')
if [ -n "$cors" ]; then

...
Optional:
  ...
  --no-tls                     Serve HTTP instead of HTTPS. Use for localhost installs or
                               behind a tunnel (cloudflared / ngrok) that terminates TLS upstream.
...
if [ "$NO_TLS" = "1" ]; then
  SCHEME="http"
  ok "Domain: $DOMAIN (HTTP mode — TLS disabled, --no-tls)"
else
  SCHEME="https"
  ok "Domain: $DOMAIN"
fi
SITE_ADDR="${SCHEME}://${DOMAIN}"
...
if [ "$NO_TLS" = "1" ]; then
  say "Starting edge (Caddy) — HTTP mode, no TLS"
  $COMPOSE up -d edge
  for i in $(seq 1 30); do
    if curl -fsS --max-time 5 "${PUBLIC_ORIGIN}/api/health" >/dev/null 2>&1; then
      ok "Edge live on ${PUBLIC_ORIGIN}"; break
    fi
    [ "$i" = "30" ] && warn "Edge not responding at ${PUBLIC_ORIGIN}. Run \`hzaconnect doctor\`."
    sleep 2
  done
else
...
${c_bold}${c_grn}=== Install complete ===${c_off}
  ...
  Embed snippet for your casino site:
    ...
    <script src="${PUBLIC_ORIGIN}/widget.js" async></script>
